...
def submit_casestudy_to_google(sender, instance, created, **kwargs):
    ...
    for lang_code in language_codes:
        try:
            activate(lang_code)
            path = reverse('case_studies:case_study_detail', kwargs={'pk': instance.pk})
...
def delete_casestudy_from_google(sender, instance, **kwargs):
    language_codes = [lang[0] for lang in settings.LANGUAGES]
    original_lang = get_language()
